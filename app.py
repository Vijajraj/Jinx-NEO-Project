...
def generate_open_response(prompt):
    payload = {
        "inputs": f"Answer concisely: {prompt}",
        "parameters": {
            "max_new_tokens": 200,
            "temperature": 0.7
        }
    ...
    if response.status_code == 200:
        ...
        return "❌ Error: Model not found — check API URL or token access."
    else:
        return f"❌ Error: {response.status_code} - {response.text}"

user_question = st.text_input("Ask a question", placeholder="E.g., What is a Near-Earth Object?")
